					...
					if ($res > 0) {
						...
						if (!$error && !empty($to)) {
							...
							if ($arraymessage->joinfiles == 1 && !empty($tmpinvoice->last_main_doc)) {
								$joinFile[] = DOL_DATA_ROOT.'/'.$tmpinvoice->last_main_doc; // Backported from 17.0
								$joinFileName[] = basename($tmpinvoice->last_main_doc);
								$joinFileMime[] = dol_mimetype(DOL_DATA_ROOT.'/'.$tmpinvoice->last_main_doc); // Backported from 17.0
							}
