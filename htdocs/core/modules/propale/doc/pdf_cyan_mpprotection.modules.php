...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_cyan_mpprotection.modules.php
 *	\ingroup    propale
 *	\brief      File of Class to generate PDF proposal with Cyan MP Protection template
 */
...
/**
 *	Class to generate PDF proposal Cyan MP Protection
 */
class pdf_cyan_mpprotection extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "cyan_mpprotection";
		$this->description = $langs->trans('DocModelCyanDescription').' (MP Protection)';
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
	{
		...
		if (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT')) {
			...
			if (getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL') && $tvaisnull) {
				...
				// Deposit (acompte) line - MP Protection specific
				$depositRate = getDolGlobalInt('PROPALE_MPPROTECTION_DEPOSIT_RATE', 40);
				if ($depositRate > 0 && $depositRate <= 100) {
					$depositAmount = price2num($total_ttc * $depositRate / 100, 'MT');

					$index++;
					$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
					$pdf->SetTextColor(0, 0, 60);
					$pdf->SetFillColor(255, 255, 255);
					$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Acompte").' ('.$depositRate.'%)', 0, 'L', 0);

					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($depositAmount, 0, $outputlangs), 0, 'R', 0);
				}
			}
		}
