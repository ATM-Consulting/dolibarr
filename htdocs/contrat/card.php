...
if (empty($reshook)) {
	...
	if ($action == 'add' && $user->rights->contrat->creer) {
		...
		if (!$error) {
			...
			if (((!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($user->hasRight('produit', 'ignore_price_min_advance')))
...
if ($action == 'create') {
	...
	if ($object->id > 0) {
		...
		// Backport this PR https://github.com/Dolibarr/dolibarr/pull/33771
