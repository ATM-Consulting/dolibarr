...
$langs->loadLangs(array("sendings", "companies", "bills", 'orders', 'stocks', 'other', 'products', 'propal', 'productbatch')); // 'products' : DYVA backport PR #40676
	...
	if ($action == 'add' && $permissiontoadd) {
		...
		if (!$error && ($qty >= 0) && (!empty($line_desc) || (!empty($idprod) && $idprod > 0))) {
			...
			if (!empty($idprod) && $idprod > 0) {
				$prod = new Product($db);
				$prod->fetch($idprod);
				$description = ''; // DYVA backport PR #40676
				// Define output language
				...
				if (getDolGlobalInt('PRODUIT_AUTOFILL_DESC') == 0) {
					$description = dol_concatdesc($description, $line_desc, false, getDolGlobalString('MAIN_CHANGE_ORDER_CONCAT_DESCRIPTION') ? true : false); // DYVA backport PR #40676
				} else {
				...
				if (!getDolGlobalString('MAIN_PRODUCT_DISABLE_CUSTOMCOUNTRYCODE') && (!empty($prod->customcode) || !empty($prod->country_code))) {
					...
					$tmptxt .= ')';
					$description = dol_concatdesc($description, $tmptxt); // DYVA backport PR #40676
				}
				$type = $prod->type;
				...
				$description = $line_desc; // DYVA backport PR #40676
				$fk_elementdet = '';
			}
			$description = dol_htmlcleanlastbr($description); // DYVA backport PR #40676
