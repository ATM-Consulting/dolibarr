...
class Expedition extends CommonObject
{
	...
	public function fetch_lines_free()
	{
		...
		/** DYVA backport PR #40676 : the product join was present but no column was read from it */
		$sql .= ', p.ref as product_ref, p.label as product_label, p.fk_product_type, p.barcode as product_barcode';
		...
		if ($result) {
			...
			while ($i < $num) {
				...
				$line->desc             = $objp->description; // DYVA backport PR #40676 - We need ->desc because some code into CommonObject use desc (property defined for other elements)
				$line->qty              = $objp->qty;
				$line->qty_shipped      = $objp->qty; // DYVA backport PR #40676 - this is a property of a shipment line, read by PDF models and getTotalWeightVolume()
				$line->fk_entrepot      = $objp->fk_entrepot;
				$line->fk_product       = $objp->fk_product;
				/** DYVA backport PR #40676 : product columns read by the PDF models */
