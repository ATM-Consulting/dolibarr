...
class Documents extends DolibarrApi
{
	...
	/**
	 * List external URL links of a Ticket (Custom AskDoli — core llx_links)
	 *
	 * Returns the standard Dolibarr object links (table llx_links) attached to a
	 * ticket, the same ones shown in the ticket Documents tab. Lets AskDoli show
	 * links in its attachments table on both sides.
	 *
	 * @param int    $id   Ticket ID
	 * @param string $ref  Ticket Ref (alternative to id)
	 * @return array       List of links: [{id, url, label, date}]
	 *
	 * @url GET /links/ticket
	 */
	public function listTicketLinks(int $id = 0, string $ref = ''): array {
		...
		if (!DolibarrApiAccess::$user->hasRight('ticket', 'read')) {
			throw new RestException(403, 'Missing permission to read ticket');
		}
		...
		$links = array();
		$linkObj->fetchAll($links, 'ticket', $object->id);
	...
	/**
	 * Add an external URL link to a Ticket (Custom AskDoli — core llx_links)
	 *
	 * Creates a standard Dolibarr object link (table llx_links) on the ticket,
	 * rendered natively in the ticket Documents tab. Used by AskDoli's "+ Lien"
	 * and by URL auto-detection in messages. Deduplicates by URL.
	 *
	 * @param int    $id    Ticket ID
	 * @param string $url   URL to link (http/https only)
	 * @param string $label Optional label (defaults to URL basename)
	 * @param string $ref   Ticket Ref (alternative to id)
	 * @return array        The created (or existing) link: {id, url, label, duplicate}
	 *
	 * @url POST /link/ticket
	 */
	public function addTicketLink(int $id = 0, string $url = '', string $label = '', string $ref = ''): array {
		require_once DOL_DOCUMENT_ROOT . '/core/class/link.class.php';
		require_once DOL_DOCUMENT_ROOT . '/ticket/class/ticket.class.php';
		...
		if (empty($url)) {
			throw new RestException(400, 'Parameter url is required');
		}
		// Only http(s) URLs (block javascript:, file:, data:, etc.)
		if (!preg_match('#^https?://#i', $url)) {
			throw new RestException(400, 'Only http(s) URLs are allowed');
		}
		if (!DolibarrApiAccess::$user->hasRight('ticket', 'write')) {
			throw new RestException(403, 'Missing permission to write ticket');
		}
		...
		// Dedup: don't add the same URL twice on this ticket
		$linkObj = new Link($this->db);
		$existing = array();
		$linkObj->fetchAll($existing, 'ticket', $object->id);
		foreach ($existing as $l) {
			if (strcasecmp(trim($l->url), $url) === 0) {
				return array('id' => (int) $l->id, 'url' => $l->url, 'label' => $l->label, 'duplicate' => true);
			}
		}

		$link = new Link($this->db);
		$link->objecttype = 'ticket';
		$link->objectid = $object->id;
		$link->url = $url;
		$link->label = $label; // create() falls back to basename(url) when empty
		$newId = $link->create(DolibarrApiAccess::$user);
		if ($newId <= 0) {
			throw new RestException(500, 'Failed to create link: ' . $link->error);
		}
		return array('id' => (int) $newId, 'url' => $link->url, 'label' => $link->label, 'duplicate' => false);
	}
