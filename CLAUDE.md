...
- `htdocs/product/class/product.class.php` + `htdocs/product/composition/card.php` + `htdocs/install/mysql/tables/llx_product_association.sql` — **composant « optionnel / de base » dans un kit** : colonne `product_association.optional` + param `$optional` (6e, après `$notrigger`) sur `add_sousproduit`/`update_sousproduit`, propagé jusqu'à `is_sousproduit`/`getChildsArbo` (index **8** — 6/7 pris par `fk_association`/`rang` en v23) / `fetch_prod_arbo` (clé `optional`). UI checkbox dans `composition/card.php`. Consommateur : trigger `cliama` `LINEORDER_INSERT` (explosion du kit à la commande, qty 0 + label « Option » pour les composants optionnels). Clé `ComposedProductOptional`.
