...
- `htdocs/product/class/product.class.php` + `htdocs/product/composition/card.php` + `htdocs/install/mysql/tables/llx_product_association.sql` + `htdocs/install/mysql/migration/22.0.0-23.0.0.sql` — **composant « optionnel / de base » dans un kit** : colonne `product_association.optional` + param `$optional` (6e, après `$notrigger`) sur `add_sousproduit`/`update_sousproduit`, propagé jusqu'à `is_sousproduit`/`getChildsArbo` (index **8** — 6/7 pris par `fk_association`/`rang` en v23) / `fetch_prod_arbo` (clé `optional`). UI checkbox dans `composition/card.php`. ALTER idempotent dans le fichier de migration `22.0.0-23.0.0.sql` (tolère « duplicate column » sur les instances déjà migrées). Consommateur : trigger `cliama` `LINEORDER_INSERT` (explosion du kit à la commande, qty 0 + label « Option » pour les composants optionnels). Clé `ComposedProductOptional`.
- `htdocs/admin/stock.php` — restaure la **bascule UI admin** de `STOCK_SHOW_ALL_BATCH_BY_DEFAULT` (repli/dépli des lots sur l'onglet stock produit), non reprise upstream en v23 alors que l'affichage l'est. Guardée `isModEnabled('productbatch')`, ajoutée après le toggle `STOCK_ALWAYS_SHOW_FULL_ARBO`. Sans elle, la const (forcée à 1) n'est modifiable qu'en base.
